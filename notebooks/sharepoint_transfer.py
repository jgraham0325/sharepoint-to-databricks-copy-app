"""
SharePoint large-file transfer (batch).
Transfers multiple files from Microsoft Graph download URLs to a Unity Catalog volume.
Downloads each file to a temp file, then uploads to the volume (avoids "File or stream is not
readable" with the Databricks Files API when streaming). Runs sequentially so cluster disk
need only hold one file at a time.
Parameters: [token, url1, path1, url2, path2, ...] (token first, then pairs of url and volume path).
"""
import os
import sys
import tempfile
